...
def _pin(t: torch.Tensor) -> torch.Tensor:
    """pin_memory() fails on nodes where CUDA driver < runtime; fall back gracefully."""
    try:
        return t.pin_memory()
    except RuntimeError:
        return t
...
class EMGDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        init_logging = os.environ.get('EMG_DATASET_INIT_LOG', '0').lower() not in ('0', 'false', 'no')
        scan_log_every = max(1, int(os.environ.get('EMG_DATASET_SCAN_LOG_EVERY', '1000')))
        t_init = time.time()
        if init_logging:
            print(
                f'[read_emg] init start dev={dev} test={test} no_testset={no_testset} '
                f'no_normalizers={no_normalizers}',
                flush=True,
            )
        ...
        if no_testset:
            ...
            if init_logging:
                print(f'[read_emg] loaded testset file in {time.time() - t:.2f}s', flush=True)
        ...
        if init_logging:
            print(f'[read_emg] indexed {len(directories)} directories in {time.time() - t:.2f}s', flush=True)

        index_cache_enabled = index_cache_path is not None and base_dir is None
        cache_loaded = False
        cache_info_files = None
        cache_scan_time_s = None
        cache_payload = None

        if index_cache_enabled:
            cp = Path(index_cache_path)
            if cp.exists():
                try:
                    cache_payload = pickle.load(open(cp, 'rb'))
                    key = cache_payload.get('key', {})
                    cache_matches = (
                        key.get('dev') == dev
                        and key.get('test') == test
                        and key.get('no_testset') == no_testset
                        and key.get('testset_file') == FLAGS.testset_file
                        and key.get('silent_data_directories') == list(FLAGS.silent_data_directories)
                        and key.get('voiced_data_directories') == list(FLAGS.voiced_data_directories)
                    )
                    if cache_matches:
                        d_by_index = {
                            d.session_index: d for d in directories
                        }
                        self.example_indices = [
                            (d_by_index[sidx], int(fidx))
                            for sidx, fidx in cache_payload['example_indices']
                            if sidx in d_by_index
                        ]
                        self.voiced_data_locations = {
                            tuple(loc): (d_by_index[sidx], int(fidx))
                            for loc, (sidx, fidx) in cache_payload['voiced_data_locations'].items()
                            if sidx in d_by_index
                        }
                        cache_loaded = True
                        if init_logging:
                            print(
                                f'[read_emg] loaded index cache {cp} '
                                f'({len(self.example_indices)} examples)',
                                flush=True,
                            )
                except Exception as exc:
                    if init_logging:
                        print(f'[read_emg] index cache load failed: {exc}', flush=True)
        ...
        if not cache_loaded:
            ...
            for directory_info in directories:
                for fname in os.listdir(directory_info.directory):
                    ...
                    if m is not None:
                        n_info_files += 1
                        if init_logging and n_info_files % scan_log_every == 0:
                            print(
                                f'[read_emg] scan progress: {n_info_files} info files '
                                f'({time.time() - t:.2f}s elapsed)',
                                flush=True,
                            )
                        idx_str = m.group(1)
            ...
            cache_info_files = n_info_files
            cache_scan_time_s = time.time() - t
            if init_logging:
                print(
                    f'[read_emg] built indices: {len(self.example_indices)} examples from '
                    f'{n_info_files} info files in {cache_scan_time_s:.2f}s',
                    flush=True,
                )
            ...
            if index_cache_enabled:
                ...
                cache_payload = {
                    'key': {
                        'dev': dev,
                        'test': test,
                        'no_testset': no_testset,
                        'testset_file': FLAGS.testset_file,
                        'silent_data_directories': list(FLAGS.silent_data_directories),
                        'voiced_data_directories': list(FLAGS.voiced_data_directories),
                    },
                    'example_indices': [
                        (ex[0].session_index, int(ex[1]))
                        for ex in self.example_indices
                    ],
                    'voiced_data_locations': {
                        tuple(loc): (ex[0].session_index, int(ex[1]))
                        for loc, ex in self.voiced_data_locations.items()
                    },
                }
                try:
                    pickle.dump(cache_payload, open(cp, 'wb'))
                    if init_logging:
                        print(f'[read_emg] saved index cache to {cp}', flush=True)
                except Exception as exc:
                    if init_logging:
                        print(f'[read_emg] index cache save failed: {exc}', flush=True)

        if cache_loaded and init_logging:
            print(
                f'[read_emg] index phase done via cache '
                f'({len(self.example_indices)} examples)',
                flush=True,
            )
        ...
        if not self.no_normalizers:
            ...
            if init_logging:
                print(f'[read_emg] loaded normalizers in {time.time() - t:.2f}s', flush=True)

        t = time.time()
        sample_mfccs, sample_emg, _, _, _, _ = load_utterance(self.example_indices[0][0].directory, self.example_indices[0][1])
        if init_logging:
            d0, i0 = self.example_indices[0]
            print(
                f'[read_emg] first load_utterance ({d0.directory}/{i0}) in {time.time() - t:.2f}s',
                flush=True,
            )
        ...
        if init_logging:
            print(f'[read_emg] init done in {time.time() - t_init:.2f}s', flush=True)
    ...
    def __getitem__(self, i):
        ...
        result = {'audio_features':_pin(torch.from_numpy(mfccs)), 'emg':_pin(torch.from_numpy(emg)), 'text':text, 'text_int': _pin(torch.from_numpy(text_int)), 'file_label':idx, 'session_ids':_pin(torch.from_numpy(session_ids)), 'book_location':book_location, 'silent':directory_info.silent, 'raw_emg':_pin(torch.from_numpy(raw_emg))}
        ...
        if directory_info.silent:
            ...
            result['parallel_voiced_audio_features'] = _pin(torch.from_numpy(voiced_mfccs))
            result['parallel_voiced_emg'] = _pin(torch.from_numpy(voiced_emg))

            audio_file = f'{voiced_directory.directory}/{voiced_idx}_audio_clean.flac'

        result['phonemes'] = _pin(torch.from_numpy(phonemes)) # either from this example if vocalized or aligned example if silent
